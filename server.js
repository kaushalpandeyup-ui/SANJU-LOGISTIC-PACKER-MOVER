...
// Consider the module "server.js" to be running in serverless mode when it is
// required by another module (require.main !== module). This is more reliable
// in deployment environments like Vercel where runtime env flags may not be
// present. Also respect explicit env flags.
const IS_VERCEL = (require.main !== module) || !!process.env.VERCEL || process.env.SERVERLESS === '1';
...
// Defensive flag to indicate whether sql.js was successfully loaded
...
async function initDb(){
  if(IS_VERCEL){
    ...
    SQL = null; SQL_LOADED = false;
  } else {
    // Non-serverless: attempt to load sql.js but be defensive - if it fails, fall back to in-memory stub
    try{
      ...
      console.error('sql.js load failed, falling back to in-memory stub:', err && (err.stack||err));
      db = {
        run: function(){ /* no-op */ },
        export: function(){ return new Uint8Array(); },
        prepare: function(){
  ...
  if(fs.existsSync(usersFile)){
    try{
      ...
      const insert = db.prepare && db.prepare('INSERT OR IGNORE INTO users (id,email,phone,passwordHash,verified,isAdmin,verifyToken) VALUES (?,?,?,?,?,?,?)');
      try{
        Object.values(raw).forEach(u=>{
          try{ if(insert){ insert.bind([u.id||uuidv4(), u.email, u.phone, u.passwordHash, u.verified?1:0, u.isAdmin?1:0, u.verifyToken||null]); insert.step(); insert.reset(); } }catch(e){ }
        });
      }finally{ try{ if(insert) insert.free(); }catch(e){} }
  ...
  if(fs.existsSync(bookingsFile)){
    try{
      ...
      const insert = db.prepare && db.prepare('INSERT OR IGNORE INTO bookings (id,uuid,userEmail,pickFrom,dropTo,createdAt,paid,amount,screenshot,paidAt,reached,reachedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
      try{
        raw.forEach(b=>{
          try{ if(insert){ insert.bind([b.id, b.uuid||uuidv4(), b.user||b.userEmail||'', b.pickFrom, b.dropTo, b.createdAt||new Date().toISOString(), b.paid?1:0, b.amount||null, b.screenshot||null, b.paidAt||null, b.reached?1:0, b.reachedAt||null]); insert.step(); insert.reset(); } }catch(e){}
        });
      }finally{ try{ if(insert) insert.free(); }catch(e){} }
  ...
  if(fs.existsSync(paymentsFile)){
    try{
      ...
      const insert = db.prepare && db.prepare('INSERT OR IGNORE INTO payments (id,bookingId,userEmail,amount,screenshot,createdAt) VALUES (?,?,?,?,?,?)');
      try{
        raw.forEach(p=>{
          try{ if(insert){ insert.bind([p.id, p.bookingId, p.user||p.userEmail||'', p.amount, p.screenshot, p.createdAt]); insert.step(); insert.reset(); } }catch(e){}
        });
      }finally{ try{ if(insert) insert.free(); }catch(e){} }
...
if(IS_VERCEL){
  ...
  module.exports = async (req, res) => {
    try{
      ...
      try{ res.statusCode = 500; res.setHeader('Content-Type','text/plain'); res.end('Server initialization error'); }catch(_){ }
    }
  };
} else {
...
// Global process error handlers - these will surface stack traces to logs instead of crashing silently
process.on('unhandledRejection', (reason, promise) => {
  console.error('unhandledRejection at:', promise, 'reason:', reason && (reason.stack || reason));
});
process.on('uncaughtException', err => {
  console.error('uncaughtException:', err && (err.stack||err));
  // in local mode we may want to exit; in serverless we just log
